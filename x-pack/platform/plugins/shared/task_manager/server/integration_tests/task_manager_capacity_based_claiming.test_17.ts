...
describe('capacity based claiming', () => {
  ...
  it('should claim tasks to full capacity', async () => {
    ...
    console.log(`*** Start injecting tasks at ${new Date().toISOString()}`);
    for (const id of ids) {
      ...
      console.log(`*** Finished injecting task ${id} at ${new Date().toISOString()}`);
      taskIdsToRemove.push(id);
    }
    console.log(`*** Stopped injecting tasks at ${new Date().toISOString()}`);
